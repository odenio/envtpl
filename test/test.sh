...
TESTDIR=$(dirname $0)
program=envtpl
cmd="go run ./cmd/envtpl/main.go"

echo "*** test with all variables set"
TAG_REGION=eu-west-1 TAG_DATACENTER=dc1 INTERVAL=2s OUTPUT_INFLUXDB_ENABLED=true HOSTNAME=localhost \
${cmd} ${TESTDIR}/test.tpl > ${TMPFILE} 2>/dev/null
...
TAG_REGION=eu-west-1 TAG_DATACENTER=dc1 INTERVAL=2s OUTPUT_INFLUXDB_ENABLED=true \
${cmd} ${TESTDIR}/test.tpl > ${TMPFILE}
...
TAG_REGION=eu-west-1 TAG_DATACENTER=dc1 INTERVAL=2s OUTPUT_INFLUXDB_ENABLED=true \
${cmd} -m zero ${TESTDIR}/test.tpl > ${TMPFILE}
...
TAG_REGION=eu-west-1 TAG_DATACENTER=dc1 INTERVAL=2s OUTPUT_INFLUXDB_ENABLED=true \
${cmd} -m error ${TESTDIR}/test.tpl > ${TMPFILE} 2>&1
if [[ $? -eq 0 ]]; then
